...
export default function ComparePage() {
  return (
    <div className="space-y-6">
      <section className="rounded-[34px] border border-white/80 bg-white p-8 shadow-panel">
        <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Cross-Shop Compare</p>
        <h1 className="mt-3 font-serif text-4xl">标准商品比价</h1>
        <p className="mt-4 max-w-3xl text-slate-600">
          所有价格都基于 AI 标准化后的 `normalizedType` 聚合，后续接入真实发布 JSON 后可直接替换数据源。
        </p>
      </section>

      <div className="grid gap-5">
        {compareGroups.map((group) => (
          <section key={group.normalizedType} className="rounded-[30px] border border-white/80 bg-white p-6 shadow-panel">
            <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
              <div>
                <div className="text-xs uppercase tracking-[0.3em] text-slate-400">product type</div>
