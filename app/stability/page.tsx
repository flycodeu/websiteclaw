...
export default function StabilityPage() {
  return (
    <div className="grid gap-6 lg:grid-cols-[420px_minmax(0,1fr)]">
      <section className="rounded-[34px] border border-white/80 bg-white p-6 shadow-panel">
        <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Methodology</p>
        <h1 className="mt-3 font-serif text-4xl">稳定度评分</h1>
        <p className="mt-4 text-slate-600">分数采用 100 分制，用于帮助判断商铺是否适合持续跟踪和采购。</p>

        <div className="mt-6 space-y-4">
          {dimensions.map((item) => (
            <div key={item.label} className="rounded-[22px] bg-shell p-4">
      ...
      <section className="rounded-[34px] border border-white/80 bg-white p-6 shadow-panel">
        ...
        <div className="mt-6 space-y-4">
          {stabilityRankings.map((entry) => (
            <div key={entry.shopId} className="rounded-[24px] bg-shell p-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  <div className="text-xs uppercase tracking-[0.22em] text-slate-400">Rank {entry.rank}</div>
