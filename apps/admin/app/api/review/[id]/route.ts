...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  if (!review) {
    return Response.json(withTraceId(null, "校对记录不存在"), { status: 404 });
  }
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> | { id: string } }) {
  try {
    ...
    return Response.json(withTraceId(review, "校对草稿已保存"));
  } catch (error) {
    return Response.json(
      withTraceId(null, error instanceof Error ? error.message : "保存校对草稿失败"),
      { status: 400 }
    );
  }
}
