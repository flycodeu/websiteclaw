import { overviewMetrics, tasks } from "@shop-claw/shared/mock-data";
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        {Object.entries(grouped).map(([status, items]) => (
          <div key={status} className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-panel">
            ...
            <div className="mt-4 space-y-3">
              {items.map((task) => (
                <div key={task.id} className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-3 text-sm text-slate-400">{task.nextAction}</div>
                </div>
              ))}
            </div>
          </div>
